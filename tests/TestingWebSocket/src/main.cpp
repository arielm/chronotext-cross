...
TEST(TestingWebSocket, basic_http_request)
{
...
void http_func(server* s, websocketpp::connection_hdl hdl) {
  ...
  EXPECT_EQ(con->get_response_code(), status_code::ok);
  EXPECT_EQ(con->get_response_msg(), status_code::get_string(status_code::ok));
}
...
void defer_http_func(server* s, bool * deferred, websocketpp::connection_hdl hdl) {
  *deferred = true;
  ...
  EXPECT_EQ(ec, websocketpp::lib::error_code());
}
...
TEST(TestingWebSocket, connection_extensions )
{
  connection_setup env(true);

  EXPECT_EQ( env.c.extension_value, 5);
  EXPECT_EQ( env.c.extension_method(), 5);

  EXPECT_TRUE( env.c.is_server() == true );
}

TEST(TestingWebSocket, basic_websocket_request )
{
  ...
  EXPECT_EQ(run_server_test(s,input), output);
}

TEST(TestingWebSocket, http_request )
{
...
TEST(TestingWebSocket, deferred_http_request )
{
  std::string input = "GET /foo/bar HTTP/1.1\r\nHost: www.example.com\r\nOrigin: http://www.example.com\r\n\r\n";
  std::string output = "HTTP/1.1 200 OK\r\nContent-Length: 8\r\nServer: ";
  output+=websocketpp::user_agent;
  ...
  EXPECT_EQ(ec, websocketpp::lib::error_code());
  EXPECT_EQ(ostream.str(), output);
  con->send_http_response(ec);
  EXPECT_EQ(ec, make_error_code(websocketpp::error::invalid_state));
}

TEST(TestingWebSocket, request_no_server_header )
{
  std::string input = "GET / HTTP/1.1\r\nHost: www.example.com\r\nConnection: upgrade\r\nUpgrade: websocket\r\nSec-WebSocket-Version: 13\r\nSec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==\r\nOrigin: http://www.example.com\r\n\r\n";
  std::string output = "HTTP/1.1 101 Switching Protocols\r\nConnection: upgrade\r\nSec-WebSocket-Accept: s3pPLMBiTxaQ9kYGzzhZRbK+xOo=\r\nUpgrade: websocket\r\n\r\n";
...
TEST(TestingWebSocket, equest_no_server_header_override )
{
  std::string input = "GET / HTTP/1.1\r\nHost: www.example.com\r\nConnection: upgrade\r\nUpgrade: websocket\r\nSec-WebSocket-Version: 13\r\nSec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==\r\nOrigin: http://www.example.com\r\n\r\n";
  std::string output = "HTTP/1.1 101 Switching Protocols\r\nConnection: upgrade\r\nSec-WebSocket-Accept: s3pPLMBiTxaQ9kYGzzhZRbK+xOo=\r\nServer: foo\r\nUpgrade: websocket\r\n\r\n";
...
TEST(TestingWebSocket, basic_client_websocket )
{
  ...
  EXPECT_EQ(ref, output.str());
}

TEST(TestingWebSocket, set_max_message_size )
{
  std::string input = "GET / HTTP/1.1\r\nHost: www.example.com\r\nConnection: upgrade\r\nUpgrade: websocket\r\nSec-WebSocket-Version: 13\r\nSec-WebSocket-Key: dGhlIHNhbXBsZSBub25jZQ==\r\n\r\n";
...
TEST(TestingWebSocket, websocket_fail_parse_error )
{
...
TEST(TestingWebSocket, websocket_fail_invalid_version )
{
...
TEST(TestingWebSocket, websocket_fail_unsupported_version )
{
...
TEST(TestingWebSocket, websocket_fail_upgrade_required )
{
  std::string input = "GET /foo/bar HTTP/1.1\r\nHost: www.example.com\r\nOrigin: http://www.example.com\r\n\r\n";
...
TEST(TestingWebSocket, client_handshake_timeout_race1 )
{
  debug_client c;
  ...
  EXPECT_EQ(con->get_ec(), make_error_code(websocketpp::error::open_handshake_timeout));
}

TEST(TestingWebSocket, client_handshake_timeout_race2 )
{
  debug_client c;
...
TEST(TestingWebSocket, server_handshake_timeout_race1 )
{
  debug_server s;
...
TEST(TestingWebSocket, server_handshake_timeout_race2 )
{
  debug_server s;
